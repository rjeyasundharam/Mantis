...
$t_query = 'SELECT O.*, I.department,P.division 
  FROM {user} O 
  LEFT JOIN {department} I ON O.department_id = I.department_id
  LEFT JOIN {division} P ON P.division_id = O.division_id
  WHERE ' . $t_where . ' ORDER BY ' . $c_sort . ' ' . $c_dir;
...
<?php
	...
	$t_columns = array(
		'username', 'realname','division','department', 'email', 'access_level',
	...
	for( $i=0; $i<$t_user_count; $i++ ) {
		...
		if( !isset( $t_access_level[$u_access_level] ) ) {
			...
			<tr>
				...
				<td><?php echo string_display_line( $u_division ) ?></td>
