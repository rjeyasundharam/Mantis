 ...
 */
function user_create( $p_username, $p_password, $p_division = '',$p_department = '',$p_email = '',
	...
	$t_query = 'INSERT INTO {user}
				    ( username, email, password, date_created, last_visit,division_id,department_id,
				     enabled, access_level, login_count, cookie_string, realname )
				  VALUES
				    ( ' . db_param() . ',' . db_param() . ',' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param()  . ',
				     ' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ', ' . db_param() . ')';
	db_query( $t_query, array( $p_username, $p_email, $t_password, db_now(), db_now(),$p_division,$p_department, $c_enabled, (int)$p_access_level, 0, $t_cookie_string, $p_realname ) );
...
/**
 * lookup the user's division in LDAP or the db as appropriate
 *
 * @param integer $p_user_id A valid user identifier.
 * @return string
 */
function user_get_division( $p_user_id ) {
	$t_query = 'SELECT d.division
						  FROM {user} u
		...
		$t_result = db_query( $t_query, array( $p_user_id) );
		$t_row = db_fetch_array( $t_result );
		return $t_row['division'];
}

/**
 * lookup the user's division_id in LDAP or the db as appropriate
 *
 * @param integer $p_user_id A valid user identifier.
 * @return string
 */
function user_get_division_id( $p_user_id ) {
	$t_query = 'SELECT division_id
						  FROM {user}  
						  WHERE id=' . db_param();

		$t_result = db_query( $t_query, array( $p_user_id) );
		$t_row = db_fetch_array( $t_result );
		return $t_row['division_id'];
}
 ...
 * @return string
 */
function user_get_department_id( $p_user_id ) {
	$t_query = 'SELECT department_id
						  FROM {user}  
						  WHERE id=' . db_param();

		$t_result = db_query( $t_query, array( $p_user_id) );
		$t_row = db_fetch_array( $t_result );
		return $t_row['department_id'];
}
