...
function prepare_user_name( $p_user_id ) {
	...
	$t_division = user_get_division( $p_user_id );
	...
	if( user_exists( $p_user_id ) && user_get_field( $p_user_id, 'enabled' ) ) {
		return '<a ' . $t_tooltip . ' href="' . string_sanitize_url( 'view_user_page.php?id=' . $p_user_id, true ) . '">' . $t_name . '</a> ('.$t_division.' - '.$t_department.')';
	}
