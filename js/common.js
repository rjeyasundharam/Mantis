...
$(document).ready( function() {
	...
	$("select#department").change(function(){
        var m_department_id = $(this).children("option:selected").val();
        var m_division_id = $("select#division").children("option:selected").val();
        
        $.post("get_user_list_by_department.php",
	    {
	      division_id: m_division_id,
	    ...
	    function(data,status){
	      ...
	      if(data.indexOf('<option') !== -1 ){
	          $("select#project-add-users-username").html(data);
		      $("select#proj-category-assigned-to").html(data);
		      $("select#handler_id").html(data);
		    }
	      else{
	...
	$("select#project-edit-division").change(function(){
        var m_division_id = $(this).children("option:selected").val();
        var m_project_id = $("#project_id").val();
        // alert("You have selected the division - " + m_division_id);
        // alert("You have selected the project - " + m_project_id);
        $.post("get_project_edit_user_list_by_division.php",
	    {
	      division_id: m_division_id,
        ...
        $.post("get_project_edit_department_list_by_division.php",
	    {
	      division_id: m_division_id
	    },
	    function(data,status){
	      // alert("Data: " + data + "\nStatus: " + status);
	      if(data.indexOf('<option') !== -1 ){
	         $("select#project-edit-department").html(data);
	      }
	      else{
	      	document.location.reload(true);	
	      }
	...
	$("select#project-edit-department").change(function(){
        var m_department_id = $(this).children("option:selected").val();
        var m_division_id = $("select#project-edit-division").children("option:selected").val();
        var m_project_id = $("#project_id").val();
        // alert("You have selected the division - " + m_division_id);
        // alert("You have selected the department - " + m_department_id);
        // alert("You have selected the project - " + m_project_id);
        
        $.post("get_project_edit_user_list_by_department.php",
	    {
	      division_id: m_division_id,
	      department_id: m_department_id,
	      project_id: m_project_id
